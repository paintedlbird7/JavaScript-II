...
//a small donation on their behalf for some much needed updates to their facilities.  
...
let largeShirts = [];
largeShirts = runners.filter((id) => { return id.shirt_size = 'L';});
...
//problems you could solve given the data set and the 5k fun run theme.  
//Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
// get potential shirt sales

shirtSoldTotal = runners.reduce((total, shirt_size) => {
    return total += company_name.shirt_size;
}, 0);
console.log(shirtSoldTotal);

// Problem 2
// get last names only

let lastName = [];
runners.forEach(item => fullName.push(`${item.last_name}`));
console.log(lastName);

// Problem 3

// get x large size only

let xlargeShirts = [];
largeShirts = runners.filter((id) => { return id.shirt_size = 'XL';});
